...
// import useCoinAudio from './hooks/useAudio';
...
// console.log('projectionPixel',projectionPixel)
  ...
  //             resultPrice,
  //             resultIndex: targetLogicalIndex,
  ...
  const handleProjectionOffsetChange = (event) => {
    ...
    if (processedAreaData.length > 0) {
      ...
      const nextTarget = currentLastIndex + safeValue;
      setTargetLogicalIndex(nextTarget);
  ...
  const handlePlaceTrade = useCallback(
    (side) => {
      if (dataSeries.length === 0 || targetLogicalIndex == null) return;

      const lastPoint = dataSeries[dataSeries.length - 1];
      const entryPrice =
        chartType === "candlestick" ? lastPoint.close : lastPoint.value;
      const lastIndex = dataSeries.length - 1;
      const barsToTarget = Math.max(targetLogicalIndex - lastIndex, 1);

      const trade = {
        id: `${Date.now()}-${Math.random().toString(16).slice(2)}`,
        side,
        entryTime: lastPoint.time,
        entryPrice,
        targetTime: lastPoint.time + barsToTarget * BAR_INTERVAL_SECONDS,
        barsToTarget,
        stake: TRADE_AMOUNT,
        status: "pending",
        resultPrice: null,
    ...
    [chartType, dataSeries, targetLogicalIndex]
  );
  ...
  useEffect(() => {
    ...
    console.log('newlyResolved',newlyResolved)
    newlyResolved.forEach(() => playBeep());
  }, [trades, dataSeries, chartType, playBeep]);
        ...
        >
          ...
          >
            {trades.map((trade) => {
              ...
              const entryLabel = `${isBuy ? "▲" : "▼"} \\$${trade.stake.toFixed(
                0
              )}`;
                ...
                <React.Fragment key={trade.id}>
                  ...
                  <div
                    style={{
                      ...
                      alignItems: "center",
                      gap: "4px",
                      boxShadow: `0 10px 24px ${isBuy ? "rgba(46,204,156,0.35)" : "rgba(239,83,80,0.35)"}`,
                      opacity: trade.status === "pending" ? 1 : 0.55,
                    }}
                  >
                    <span
                      style={{
